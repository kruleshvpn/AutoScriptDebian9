...
data=$( curl -s script.kyzoproject.tk/License/auth.txt | grep -E $ip | head -n1 );
if [[ $ip == "" ]]; then
    echo "Sorry Have Something Eror !"
    exit 1
elif [[ $ip == $data ]]; then
    echo -e "${GREEN}Passed ! ${NC}( Your IP Address Is Registered )"
else
    sleep 1
    echo -e "${RED}Eror !${NC} ( Your IP Address Not Registered )"
    exit 1
fi
IP=$( curl -s ipv4.wildyproject.my.id );
clear
